...
with g.as_default():
    X = tf.placeholder(dtype=tf.float32, shape=[BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, 2], name='input')
    Y_ = tf.placeholder(dtype=tf.float32, shape=[None, NUM_LABELS])

    conv1 = tf.nn.conv2d(X, tf.get_variable(name='conv1', shape=[CONV1_SIZE, CONV1_SIZE, 2, CONV1_DEEP],
                                            dtype=tf.float32,
                                            initializer=tf.truncated_normal_initializer(0.1, stddev=0.01)),
                         strides=[1, 1, 1, 1],
                         padding="SAME")
    BN = tf.layers.batch_normalization(conv1, training=True)
    bias1 = tf.nn.relu(tf.nn.bias_add(BN, tf.get_variable('bias1', shape=[CONV1_DEEP], dtype=tf.float32,
    ...
    bias2 = tf.nn.relu(tf.nn.bias_add(BN, tf.get_variable('bias2', shape=[CONV2_DEEP], dtype=tf.float32,
    ...
    BN = tf.layers.batch_normalization(fcn1, training=True)
    relu1 = tf.nn.relu(BN)
    ...
    BN = tf.layers.batch_normalization(fcn2, training=True)
    relu2 = tf.nn.relu(BN)
...
def get_data(path, train=True, batch=BATCH_SIZE):
    ...
    selected_name = []
    selected_label = []
    selected_data = np.zeros([batch, IMAGE_SIZE, IMAGE_SIZE, 2])
    count = 0
    for i in range(len(x)):
        selected_name.append(data[x[i], y[i]])
        selected_label.append(label[x[i], y[i]].tolist())
        img = cv2.imread('./yale_face/' + data[x[i], y[i]], 0)
        selected_data[count, ..., 0] = img
        selected_data[count, ..., 1] = local_binary_pattern(img, 24, 3)
        count += 1
...
if __name__ == '__main__':
    with tf.Session(graph=g) as sess:
        ...
        summary_writer1 = tf.summary.FileWriter('./summary_5/train', graph=sess.graph)
        summary_writer2 = tf.summary.FileWriter('./summary_5/test', graph=sess.graph)
