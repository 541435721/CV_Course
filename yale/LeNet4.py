...
def get_data(path, train=True, batch=BATCH_SIZE):
    ...
    for i in range(len(x)):
        ...
        selected_data[count, :, :] = local_binary_pattern(img, 24, 3)
        count += 1
...
if __name__ == '__main__':
    with tf.Session(graph=g) as sess:
        ...
        summary_writer1 = tf.summary.FileWriter('./summary_4/train', graph=sess.graph)
        summary_writer2 = tf.summary.FileWriter('./summary_4/test', graph=sess.graph)
